-- cool3dbackground "class"
local Cool3dBg = {}
Cool3dBg.__index = Cool3dBg

function Cool3dBg.new(debugmode)
	local self = setmetatable({}, Cool3dBg)
...
function Cool3dBg:readFile(filename)
...
function Cool3dBg:translate_z(xyz, dz)
	local x = xyz[1]
	local y = xyz[2]
	local z = xyz[3]
	return {x, y, z + dz}
...
function Cool3dBg:rotate_xz(xyz, angle)
	local x = xyz[1]
	local y = xyz[2]
	local z = xyz[3]
